...
async def root():
    ...
    if topology_type == "Sharded":
        shards = await get_documents_count_per_shard()
...
async def get_documents_count_per_shard() -> dict:
    shards_info = await client.admin.command("listShards")

    result = {"shards": {}}

    for shard in shards_info["shards"]:
        shard_id = shard["_id"]      # shard1
        host_string = shard["host"]  # shard1/shard1:27018,...

        # формат: replicaSetName/host1,host2,...
        rs_name, hosts = host_string.split("/")

        shard_client = motor.motor_asyncio.AsyncIOMotorClient(
            f"mongodb://{hosts}/?replicaSet={rs_name}"
        )

        result["shards"][shard_id] = {"hosts": f"{host_string}", "collections": {}}

        collection_names = await db.list_collection_names()
        for collection_name in collection_names:
            documents_count = await shard_client[DATABASE_NAME][collection_name].count_documents({})

            result["shards"][shard_id]["collections"] = {collection_name: (
